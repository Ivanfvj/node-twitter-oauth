<template>
  <div class="callback-page">
    ...
    <div v-if="error">
      <div v-if="callbackError" class="text-left my-5 max-w-3xl">
        ...
        <div class="space-y-2">
          ...
          <p>
            This page expect
            <u>oauth_token and oauth_verifier query params</u> from
            <a
    ...
    >
      ...
      <div class="my-5">
        <p>The oauth_token and oauth_verifier from callbackUrl</p>
        <p>{{ oauth_token }}</p>
        <p>{{ oauth_verifier }}</p>
      </div>
      <div>
      ...
      <!-- <button @onClick.stop="generateTokenAndLogin">Consultar</button> -->
    </div>
  </div>
</template>
...
export default Vue.extend({
  ...
  mounted() {
    // Get oauth_token and oauth_verifier from url query (twitter callbackUrl)
  ...
  methods: {
    ...
    async generateTokenAndLogin() {
      if (this.loading) return;
      this.loading = true;
      try {
        // Request accessToken exchange to the backend service
        const response = await api.twitter.generateAccessTokenAndLogin(
          this.oauth_token,
          this.oauth_verifier
        );
